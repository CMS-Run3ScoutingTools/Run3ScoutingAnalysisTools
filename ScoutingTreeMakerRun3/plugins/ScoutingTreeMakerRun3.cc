...
private:
  ...
  // Objects
  const edm::EDGetTokenT<std::vector<Run3ScoutingMuon> >      muonsNoVtxToken;
  const edm::EDGetTokenT<std::vector<Run3ScoutingMuon> >      muonsVtxToken;
...
ScoutingTreeMakerRun3::ScoutingTreeMakerRun3(const edm::ParameterSet& iConfig):
  ...
  l1GtUtils_(nullptr),
  muonsNoVtxToken          (consumes<std::vector<Run3ScoutingMuon> >             (iConfig.getParameter<edm::InputTag>("muonsNoVtx"))),
  muonsVtxToken            (consumes<std::vector<Run3ScoutingMuon> >             (iConfig.getParameter<edm::InputTag>("muonsVtx"))),
...
void ScoutingTreeMakerRun3::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  Handle<vector<Run3ScoutingMuon> > muonsNoVtxH;
  iEvent.getByToken(muonsNoVtxToken, muonsNoVtxH);
  Handle<vector<Run3ScoutingMuon> > muonsVtxH;
  iEvent.getByToken(muonsVtxToken, muonsVtxH);
  ...
  int j=0;
  for (auto muons_iter = muonsNoVtxH->begin(); muons_iter != muonsNoVtxH->end(); ++muons_iter) {
  ...
  if (idx.size()>1) {
    nMuonsNoVtx = idx.size();
    //std::cout << "charge: " << (muonsH->at(idx[0]).charge()) << ", " << (muonsH->at(idx[1]).charge()) << std::endl;
    if ((muonsNoVtxH->at(idx[0]).charge())*(muonsNoVtxH->at(idx[1]).charge()) > 0) {
      return;
    }
    
    //muonID1 = (muonsNoVtxH->at(idx[0]).pt()>4) && (muonsNoVtxH->at(idx[0]).trackIso()<0.15) && (muonsNoVtxH->at(idx[0]).nValidPixelHits()>0) && (muonsNoVtxH->at(idx[0]).trk_chi2()<10);
    //muonID2 = (muonsNoVtxH->at(idx[1]).pt()>4) && (muonsNoVtxH->at(idx[1]).trackIso()<0.15) && (muonsNoVtxH->at(idx[1]).nValidPixelHits()>0) && (muonsNoVtxH->at(idx[1]).trk_chi2()<10);
    
    //std::cout << "ID" << muonID1 << ", " << muonID2 << std::endl;
    
    trackIso1 = muonsNoVtxH->at(idx[0]).trackIso();
    trackIso2 = muonsNoVtxH->at(idx[1]).trackIso();   
    nValidPixelHits1 = muonsNoVtxH->at(idx[0]).nValidPixelHits();
    nValidPixelHits2 = muonsNoVtxH->at(idx[1]).nValidPixelHits();
    nTrackerLayersWithMeasurement1 = muonsNoVtxH->at(idx[0]).nTrackerLayersWithMeasurement();
    nTrackerLayersWithMeasurement2 = muonsNoVtxH->at(idx[1]).nTrackerLayersWithMeasurement();
    trk_chi21 = muonsNoVtxH->at(idx[0]).trk_chi2();
    trk_chi22 = muonsNoVtxH->at(idx[1]).trk_chi2();
    
    pt1 = muonsNoVtxH->at(idx[0]).pt();
    pt2 = muonsNoVtxH->at(idx[1]).pt();
    eta1 = muonsNoVtxH->at(idx[0]).eta();
    eta2 = muonsNoVtxH->at(idx[1]).eta();      
    phi1 = muonsNoVtxH->at(idx[0]).phi();
    phi2 = muonsNoVtxH->at(idx[1]).phi();
    ...
    nMuonsVtx = muonsVtxH->size();
    if (muonsVtxH->size()>= 2){      
      pt1_muVtx = muonsVtxH->at(idx[0]).pt();
      pt2_muVtx = muonsVtxH->at(idx[1]).pt();
      eta1_muVtx = muonsVtxH->at(idx[0]).eta();
      eta2_muVtx = muonsVtxH->at(idx[1]).eta();      
      phi1_muVtx = muonsVtxH->at(idx[0]).phi();
      phi2_muVtx = muonsVtxH->at(idx[1]).phi();
      
      TLorentzVector muVtx1;
      TLorentzVector muVtx2;
      muVtx1.SetPtEtaPhiM(pt1_muVtx,eta1_muVtx,phi1_muVtx,0.105658);	
      muVtx2.SetPtEtaPhiM(pt2_muVtx,eta2_muVtx,phi2_muVtx,0.105658);      
      TLorentzVector dimuVtx = muVtx1+muVtx2;
      mass_muVtx=dimuVtx.M();
      pt_muVtx=dimuVtx.Pt();
      dr_muVtx=muVtx1.DeltaR(muVtx2);
    }
    else{
      pt1_muVtx = 0;
      pt2_muVtx = 0;
    ...
    std::vector<int> vtxIndx1 = (muonsNoVtxH->at(idx[0])).vtxIndx();
    std::vector<int> vtxIndx2 = (muonsNoVtxH->at(idx[1])).vtxIndx();
...
void ScoutingTreeMakerRun3::beginJob() {
  ...
  tree->Branch("nMuonsNoVtx", &nMuonsNoVtx, "nMuonsNoVtx/I");
  tree->Branch("nMuonsVtx", &nMuonsVtx, "nMuonsVtx/I");
  ...
  tree->Branch("mass_muVtx"          , &mass_muVtx                  , "mass_muVtx/F"    );
  tree->Branch("pt_muVtx"            , &pt_muVtx                    , "pt_muVtx/F"      );
  tree->Branch("dr_muVtx"            , &dr_muVtx                    , "dr_muVtx/F"      );
  tree->Branch("pt1_muVtx"           , &pt1_muVtx                   , "pt1_muVtx/F"     );
  tree->Branch("pt2_muVtx"           , &pt2_muVtx                   , "pt2_muVtx/F"     );
  tree->Branch("eta1_muVtx"          , &eta1_muVtx                  , "eta1_muVtx/F"    );
  tree->Branch("eta2_muVtx"          , &eta2_muVtx                  , "eta2_muVtx/F"    );
  tree->Branch("phi1_muVtx"          , &phi1_muVtx                  , "phi1_muVtx/F"    );
  tree->Branch("phi2_muVtx"          , &phi2_muVtx                  , "phi2_muVtx/F"    );
